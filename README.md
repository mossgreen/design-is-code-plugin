...
- **Pure-function leaves** — pinned at every decision-table row and at every declared **boundary** (a threshold demonstrated by a bracketing pair of rows, e.g. quantity `4` → 0% and quantity `5` → 10% pin the tier cut at exactly 5; DisC refuses a declared boundary without its pair). Enum and boolean inputs are pinned by **finite-domain coverage**: every value of the domain must have a row — DisC refuses an uncovered value — so a finite column has no between-rows gap at all. Between rows of a numeric column, only declared boundaries are verified. The remaining human duties — every business threshold appears in `boundaries:`, and the rows cover the combinations that matter across multiple finite columns — are checklist items at sign-off, not code archaeology after generation.
